...
func InitHandle(conf option.Config) error {
	...
	defaultServieHandler = CreateManager(conf, mqClient, etcdcli, statusCli, prometheusCli, rainbondClient, clientset, kubevirtCli, dbmanager, registryCli, restconfig)
	...
	defaultGatewayHandler = CreateGatewayManager(dbmanager, mqClient, etcdcli, gatewayClient, clientset, restconfig)
	...
	defApplicationHandler = NewApplicationHandler(conf, restconfig, statusCli, prometheusCli, rainbondClient, clientset, dynamicClient, kruiseClient, gatewayClient)
